...
export const ProfessionalQuestionsTallyGrid: React.FC<ProfessionalQuestionsTallyGridProps> = ({
  ...
  const renderWrittenCounterCard = (
    ...
    countBadgeColor: string = 'bg-sky-950 text-sky-300 border-sky-800/60'
  ) => {
      ...
      <div className={`p-4 rounded-2xl bg-[#0e1422] border border-slate-800 space-y-2.5 flex flex-col justify-between hover:border-slate-700 transition-colors ${colSpan}`}>
        <div>
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-white leading-snug">{title}</h3>
            <span className={`px-2 py-0.5 rounded border text-[10px] font-mono font-bold shrink-0 ml-2 ${countBadgeColor}`}>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {renderWrittenCounterCard(
            ...
            professionalSurveys.map((p) => p.q36 || p.pitfallsToAvoid),
            'col-span-1',
            'bg-amber-400/20 text-[#F3BA4F]',
            'bg-amber-950 text-amber-300 border-amber-800/60'
          )}
