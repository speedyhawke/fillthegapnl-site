...
export const CommunityQuestionsTallyGrid: React.FC<CommunityQuestionsTallyGridProps> = ({
  ...
  const renderSingleChoiceCard = (
    ...
    return (
      <div className="p-4 rounded-2xl bg-[#0e1422] border border-slate-800 space-y-2.5 flex flex-col justify-between hover:border-slate-700 transition-colors">
        <div>
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-1.5 mb-2">
            <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-black uppercase ${badgeColor}`}>
              {qNumber}
            </span>
            <span className="text-[10px] text-stone-400 font-bold uppercase truncate max-w-[140px] text-right">
              {badgeText}
            </span>
          </div>
        ...
        <div className="space-y-1.5 text-xs pt-1 max-h-56 overflow-y-auto pr-1">
          {entries.length === 0 ? (
            <div className="text-[11px] text-stone-500 italic p-2 bg-slate-900/50 rounded-lg">No responses yet</div>
          ) : (
            entries.map(([opt, count]) => {
              ...
              return (
                <div
                  key={opt}
                  className="p-2 rounded-lg bg-slate-900/90 border border-slate-800/60 flex items-center justify-between gap-2 hover:bg-slate-800/60 transition-colors"
                >
                  <span className="text-slate-300 font-medium truncate text-xs" title={opt}>
                    {opt}
                  </span>
                  <div className="flex items-center gap-1.5 shrink-0 font-mono text-xs">
                    <span className="text-white font-bold">{count}</span>
                    <span className={`${countColor} text-[11px]`}>({pct}%)</span>
                  </div>
                </div>
              );
            })
  ...
  const renderMultiChoiceCard = (
    ...
    return (
      <div className={`p-4 rounded-2xl bg-[#0e1422] border border-slate-800 space-y-2.5 flex flex-col justify-between hover:border-slate-700 transition-colors ${colSpan}`}>
        <div>
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-1.5 mb-2">
            <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-black uppercase ${badgeColor}`}>
              {qNumber}
            </span>
            <span className="text-[10px] text-stone-400 font-bold uppercase truncate max-w-[180px] text-right">
              {badgeText}
            </span>
          </div>
          <h3 className="text-xs font-bold text-white leading-snug">{title}</h3>
        ...
        <div className="space-y-1.5 text-xs pt-1 max-h-64 overflow-y-auto pr-1">
          {entries.length === 0 ? (
            <div className="text-[11px] text-stone-500 italic p-2 bg-slate-900/50 rounded-lg">No responses yet</div>
          ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1.5 truncate">
                    <span className="text-[10px] font-mono font-bold text-stone-500 w-4 shrink-0">
                      #{idx + 1}
                    </span>
                    <span className="text-slate-200 font-medium truncate text-xs" title={opt}>
                      {opt}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0 font-mono text-xs">
  ...
  const renderScaleCard = (
    ...
    return (
      <div className="p-4 rounded-2xl bg-[#0e1422] border border-slate-800 space-y-2.5 flex flex-col justify-between hover:border-slate-700 transition-colors">
        <div>
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-1.5 mb-2">
            <span className="px-2 py-0.5 rounded text-[10px] font-mono font-black uppercase bg-purple-400/20 text-purple-300">
              {qNumber}
            </span>
            <span className="text-[10px] text-stone-400 font-bold uppercase truncate max-w-[140px] text-right">
              {badgeText}
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-white leading-snug">{title}</h3>
            <span className="px-2 py-0.5 rounded bg-purple-950 text-purple-300 border border-purple-800/60 text-[10px] font-mono font-bold shrink-0 ml-2">
              Avg: {avg} / 5
            </span>
          </div>
        </div>
        ...
        <div className="space-y-1.5 text-xs pt-1">
          {scale.map((num) => {
            ...
            return (
              <div
                key={num}
                className="p-2 rounded-lg bg-slate-900/90 border border-slate-800/60 flex items-center justify-between gap-2"
              >
                <span className="text-slate-300 font-medium text-xs">{labels[num] || `${num} Stars`}</span>
                <div className="flex items-center gap-1.5 shrink-0 font-mono text-xs">
                  <span className="text-white font-bold">{count}</span>
                  <span className="text-purple-400 text-[11px]">({pct}%)</span>
                </div>
              </div>
            );
          })}
  ...
  const renderWrittenCounterCard = (
    ...
    return (
      <div className={`p-4 rounded-2xl bg-[#0e1422] border border-slate-800 space-y-2.5 flex flex-col justify-between hover:border-slate-700 transition-colors ${colSpan}`}>
        <div>
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-1.5 mb-2">
            <span className="px-2 py-0.5 rounded text-[10px] font-mono font-black uppercase bg-sky-400/20 text-sky-300">
              {qNumber}
            </span>
            <span className="text-[10px] text-stone-400 font-bold uppercase truncate max-w-[140px] text-right">
              {badgeText}
          ...
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-white leading-snug">{title}</h3>
            <span className="px-2 py-0.5 rounded bg-sky-950 text-sky-300 border border-sky-800/60 text-[10px] font-mono font-bold shrink-0 ml-2">
        ...
        <div className="space-y-1.5 pt-1 text-xs">
          {recent.length === 0 ? (
            <div className="text-[11px] text-stone-500 italic p-2 bg-slate-900/50 rounded-lg">No written entries yet</div>
          ) : (
            recent.map((text, i) => (
              <div key={i} className="p-2 rounded-lg bg-slate-900/90 border border-slate-800/60 text-slate-300 italic text-[11px] line-clamp-2">
                "{text}"
              </div>
            ))
          )}
